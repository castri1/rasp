...
import {
  ...
  Desktop, CornersOut, Info, Leaf, LockSimple, MapPin, GearSix, HouseLine, Lamp, ListChecks, MoonStars, Timer, Pause, Play, Plus,
...
import NightWallpaper from './NightWallpaper';
import { meetingBlocksWallpaper, shouldShowNightWallpaper } from './wallpaper';
...
type AppView = 'agenda' | 'calendar' | 'tasks' | 'alarms' | 'pomodoro' | 'home' | 'ambience' | 'wallpaper' | 'settings';
...
function MeetAction({ event, available, focusReady, demo, status, onOpen }: { event: CalendarEvent; available: boolean; focusReady: boolean; demo: boolean; status: OpenStatus; onOpen: (event: CalendarEvent) => void }) {
  ...
  return <div className="meet-action">
    <button className={`primary-button ${opened ? 'is-opened' : ''}`} onClick={() => onOpen(event)} disabled={!available || opening}>
      ...
      <span>{!available ? 'Mac no disponible' : opening ? 'Preparando Mac…' : opened ? demo ? 'Abierto · simulación' : 'Abierto en tu Mac' : focusReady ? 'Abrir + No molestar' : 'Abrir en mi Mac'}</span>
...
function eventDateLabel(value?: string) {
  ...
  return date.toLocaleDateString('es-CO', { weekday: 'short', day: 'numeric', month: 'short' }).replace('.', '');
}

function EventDetails({ event, ended, available, focusReady, demo, status, muted, onClose, onOpen, onMute }: {
  event: CalendarEvent; ended: boolean; available: boolean; focusReady: boolean; demo: boolean; status: OpenStatus; muted: boolean;
  ...
  return <div className="detail-backdrop">
    <div className="event-details" role="dialog" aria-modal="true" aria-labelledby="detail-title" ref={dialogRef} onKeyDown={event => {
      ...
      <div className="detail-top"><span className="eyebrow">DETALLE DE TU AGENDA</span><button ref={closeRef} className="icon-button" onClick={onClose} aria-label="Cerrar detalles"><X size={23} /></button></div>
      <div className="detail-content">
        <span className="detail-time">{eventDateLabel(event.date) && <>{eventDateLabel(event.date)} <span>·</span> </>}{formatTime(event.start * 60)} <span>–</span> {formatTime(event.end * 60)} <span>· {duration(event)}</span></span>
      ...
      <div className="detail-actions">
        {!ended ? <MeetAction event={event} available={available} focusReady={focusReady} demo={demo} status={status} onOpen={onOpen} /> : <div className="ended-note"><CheckCircle size={21} />Esta reunión ya terminó</div>}
        {!ended && <button className="secondary-button" onClick={() => onMute(event.id)}>{muted ? <Check size={19} /> : <BellSimpleSlash size={19} />}{muted ? 'Aviso silenciado' : 'Silenciar este evento'}</button>}
      </div>
    </div>
  </div>;
}

function Agenda({ events, seconds, featured, onSelect, onExpand, emptyMessage = <>Sin compromisos.<br />Con posibilidades.</> }: { events: CalendarEvent[]; seconds: number; featured?: CalendarEvent; onSelect: (event: CalendarEvent) => void; onExpand: () => void; emptyMessage?: React.ReactNode }) {
...
function Device({ alexa, home, calendar, calendarMode, reminders, alarms, screenLock, initialScene, onAlexaSettings, onCalendarSettings, onSessionStart, view, onView, theme, pomodoro, mac, wallpaper, wallpaperVisible, onWallpaperExit, settingsSection, onSection, events, seconds, scenario, selected, setSelected, acknowledged, onAcknowledge, pomodoroNoticeDismissed, onDismissPomodoroNotice, muted, onMute, onOpen, openStatus, notice, onNotice }: {
  ...
  alarms: AlarmsController; screenLock: ScreenLockController;
  ...
  if (screenLock.locked) return <section className="device locked-device" aria-label="Pantalla bloqueada" data-testid="device"><LockScreen controller={screenLock} /><AlarmOverlay controller={alarms} /></section>;
  if (wallpaperVisible) return <section className="device wallpaper-device" aria-label="Fondo nocturno animado" data-testid="device"><NightWallpaper config={wallpaper.config} onExit={onWallpaperExit} /><AlarmOverlay controller={alarms} /></section>;
  ...
  return <section className={`device ${isAlert && view === 'agenda' ? 'alert-mode' : ''} ${view === 'settings' ? 'settings-mode' : ''} ${isDone || isFree ? 'quiet-mode' : ''}`} aria-label="Pantalla del reloj de reuniones" data-testid="device">
    ...
    {view === 'settings' ? <SettingsScreen theme={theme} mac={mac} alexa={alexa} calendar={calendar} wallpaper={wallpaper} screenLock={screenLock} onWallpaperPreview={() => onView('wallpaper')} initialScene={initialScene} section={settingsSection} onSection={onSection} /> : view === 'calendar' ? <CalendarScreen calendar={calendar} demo={calendarMode === 'demo'} seconds={seconds} onBack={() => onView('agenda')} onSelect={setSelected} /> : view === 'tasks' ? <TasksScreen controller={reminders} demo={calendarMode === 'demo'} /> : view === 'alarms' ? <AlarmsScreen controller={alarms} /> : view === 'home' ? <HomeScreen controller={home} /> : view === 'ambience' ? <AmbienceScreen alexa={alexa} onSettings={onAlexaSettings} /> : view === 'pomodoro' ? <PomodoroScreen onSessionStart={onSessionStart} pomodoro={pomodoro} mac={mac} onMacSettings={() => { onSection('mac'); onView('settings'); }} /> : isAlert && featured ? <main className={`alert-content ${alertEvents.length > 1 ? 'has-overlap' : ''}`} key="alert">
      ...
      <div className="alert-actions"><MeetAction event={featured} available={macAvailable} focusReady={Boolean(mac.status?.focusReady)} demo={calendarMode === 'demo'} status={openStatus} onOpen={onOpen} /><button className="secondary-button" onClick={() => onMute(featured.id)}><BellSimpleSlash size={20} />Silenciar este evento</button></div>
      ...
      <div className="day-main"><ClockFace seconds={seconds} subtitle={calendarUnavailable ? calendarMode === 'loading' ? 'Preparando tu agenda.' : 'Tu calendario está por conectar.' : subtitle} />
        {calendarUnavailable ? <button className="calendar-disconnected-card" onClick={onCalendarSettings}><span className="eyebrow">GOOGLE WORKSPACE</span><strong>{calendarMode === 'loading' ? 'Consultando tu calendario…' : 'Conecta tu agenda.'}</strong><span>{calendarMode === 'loading' ? 'Un momento.' : 'Autoriza el acceso desde Ajustes.'}</span><ArrowRight size={18} /></button> : featured ? <div className={`featured-meeting ${current ? 'in-progress' : ''}`}>
          ...
          <MeetAction event={featured} available={macAvailable} focusReady={Boolean(mac.status?.focusReady)} demo={calendarMode === 'demo'} status={openStatus} onOpen={onOpen} />
    ...
    <AlarmOverlay controller={alarms} />
  </section>;
}
...
export default function App() {
  ...
  const wallpaperBlocked = meetingBlocksWallpaper(events, visibleSeconds, wallpaper.config.wakeBeforeMinutes);
  const wallpaperAutomatic = shouldShowNightWallpaper({
    config: wallpaper.config,
    events,
    seconds: visibleSeconds,
    calendarReady: calendarMode === 'connected' || calendarMode === 'demo' && scenario === 'night',
    pomodoroActive: pomodoro.timer.status === 'running' || pomodoro.timer.status === 'paused',
  });
  const wallpaperVisible = !wallpaperBlocked && (view === 'wallpaper' || view === 'agenda' && wallpaperAutomatic && Date.now() >= wallpaperSnoozedUntil);
  ...
  useEffect(() => {
    ...
    setView('agenda');
    setNotice('La agenda volvió porque hay una reunión cerca.');
  }, [view, wallpaperBlocked]);
  ...
  function changeScenario(next: Scenario) {
    resetInteraction(); setView('agenda'); setWallpaperSnoozedUntil(0); setScenario(next); setSeconds(scenarios.find(item => item.id === next)!.time * 60);
  }
  ...
  async function openMeeting(event: CalendarEvent) {
    ...
    try {
      const deadline = event.endAt ? new Date(event.endAt) : new Date();
      if (!event.endAt) deadline.setHours(Math.floor(event.end / 60), event.end % 60, 0, 0);
      const message = await mac.openMeet(event.meetUrl, deadline.getTime());
      setOpenStatus({ id: event.id, state: 'opened' });
      setAcknowledged(previous => new Set(previous).add(event.id));
      setNotice(message);
    } catch (error) {
