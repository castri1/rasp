...
import { ArrowLeft, ArrowsClockwise, Check, HouseLine, Lightbulb, LinkSimple, PencilSimple, Plus, Power, SlidersHorizontal, Trash, WarningCircle } from '@phosphor-icons/react';
...
function roomStatus(room: { devices: HomeDevice[]; on: number; unavailable: number }) {
...
export default function HomeScreen({ controller }: { controller: HomeController }) {
  ...
  function openEditor() {
    setDraftRooms(home.rooms.map(room => ({ id: room.id, name: room.name, slot: room.slot })));
  ...
  function addRoom() {
    ...
    setDraftRooms(previous => [...previous, { id, name: 'Nuevo ambiente', slot: 'other' }]);
  ...
  return <main className="home-screen app-content" aria-label="Plano y control de la casa">
    ...
    <div className="home-layout">
      ...
      {selectedRoom && <aside className="home-room-panel" aria-label={`Dispositivos de ${selectedRoom.name}`}>
        <div className="home-room-panel-heading"><select aria-label="Ambiente seleccionado" value={selectedRoom.id} onChange={event => setSelectedRoomId(event.target.value)}>{home.rooms.map(room => <option key={room.id} value={room.id}>{room.name}</option>)}</select><button aria-label={`${selectedRoom.on ? 'Apagar' : 'Encender'} ${selectedRoom.name}`} disabled={!selectedRoom.devices.length || busyRoom !== null} onClick={() => void controller.toggle(selectedRoom.id, !selectedRoom.on)}><Power size={16} /></button></div>
        ...
        <div className="home-device-list">
          ...
          {!selectedRoom.devices.length && <div className="home-room-empty"><Lightbulb size={22} weight="thin" /><span>Asocia aquí tus dispositivos desde <b>Editar</b>.</span></div>}
        </div>
      </aside>}
    </div>
